...
FONT="Helvetica"
COLORLINKS="true"
pandoc     --from=markdown     --filter=pandoc-eqnos     --filter=pandoc-tablenos     --filter=pandoc-img-glob     --filter=pandoc-chemfig     --filter=pandoc-fignos     --bibliography=$BIBLIOGRAPHY_PATH     --csl=$CSL_PATH     --template=build/assets/nih4.tex     --metadata link-citations=true     --number-sections     --resource-path=.:content:../content     --pdf-engine=xelatex     --variable mainfont="${FONT}"     --variable sansfont="${FONT}"     --variable colorlinks="${COLORLINKS}"     --output=diff/output/manuscript.pdf     $INPUT_PATH
echo "Build complete"
